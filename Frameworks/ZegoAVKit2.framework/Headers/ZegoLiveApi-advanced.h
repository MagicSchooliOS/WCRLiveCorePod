...
- (void)setVideoEncoderRateControlStrategy:(ZegoAPIVideoEncoderRateControlStrategy)strategy encoderCRF:(int)crf;
 ...
 */
- (bool)setVideoCodecId:(ZegoVideoCodecAvc)codecId;
...
/**
 ...
 @attention 仅拉 UDP 流有效
 */
- (int)activateVideoPlayStream:(int)index active:(bool)active;
 ...
 */
- (int)activateVideoPlayStream:(int)index active:(bool)active videoLayer:(VideoStreamLayer)videoLayer;
...
/// \brief 设置TrafficControl视频码率最小值
/// \param bitrate 码率，单位为bps
/// \param mode 模式，低于最低码率时视频发送模式
/// \attention InitSDK 之后调用有效
/// \note 设置一个在traffic control中video码率的一个最小值，当网络不足以发送这个最小值的时候视频会被卡住，而不是以低于该码率继续发送。初始化SDK后默认情况下没有设置改值，即尽可能的保持视频流畅，InitSDK之后可以随时修改，未重新InitSDK之前如果需要取消该设置值的限制可以设置为0
